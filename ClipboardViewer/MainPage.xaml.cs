    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async void Page_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            await Windows.ApplicationModel.Core.CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
            {
                ...
                WriteLog("开始获取数据，来源：剪贴板；类型：启动");
                await GetData(data);
                WriteLog("获取数据结束" + Environment.NewLine);
            });
        }
        ...
        private async void ClipboardContentChangedEventHandler(object sender, object e)
        {
                ...
                await Windows.ApplicationModel.Core.CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
                {
                    ...
                    {
                        WriteLog("检测到剪贴板数据变化，但是程序位于后台无法捕获。" + Environment.NewLine);
                        hasData = true;
                        return;
                    }
                    WriteLog("开始获取数据，来源：剪贴板；类型：实时");
                    await GetData(data);
                WriteLog("获取数据结束" + Environment.NewLine);
                });
        ...
        private async Task GetData(DataPackageView data)
        {
            ...
            txtAvaliable.Text = "";
            foreach (var i in data.AvailableFormats)
            {
                txtAvaliable.Text += i + Environment.NewLine;
            }
        ...
        private async void btnImg_Click(object sender, RoutedEventArgs e)
        {
            //try
            //{
            //    if (await PickAndSaveFile(ImageBytes, new Dictionary<string, string>() { { "位图", ".bmp" } }))
            //    {
